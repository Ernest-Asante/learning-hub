...
function IctAnswers() {
  const [data, setData] = useState('')
    ...
    useEffect(()=>{ 
      const  postRef=collection(db,"answers", "ict" ,"ict");
   ...
   <Box sx={{marginTop:"15%"}}>
    <Typography variant="h6">SELECT THE TEST...</Typography>
     ...
     { data && data.map(({ begin,id,createdAt, title})=>( 
           ...
           <Card  onClick={()=> navigate(`/answerdetail4/${id}`)}  key={{id}} sx={{minHeight:"120px", width:"98%", margin:"5px", marginRight:"1%"}}>
