...
function ScienceAnswers() {
  const [data, setData] = useState('')
    ...
    useEffect(()=>{ 
      const  postRef=collection(db,"answers", "science" ,"science");
  ...
  return (
   ...
   <Box sx={{marginTop:"15%"}}>
    <Typography variant="h6">SELECT THE TEST...</Typography>
     ...
     { data && data.map(({ title,id,createdAt,begin})=>( 
           <Box sx={{marginTop:"10px"}}> 
           <Card  onClick={()=> navigate(`/answerdetail1/${id}`)}  key={{id}} sx={{minHeight:"80px", width:"98%", margin:"5px", marginRight:"1%"}}>
