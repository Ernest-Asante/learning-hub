...
function QuestionDetail2() {
  ...
  const { id } = useParams(); 
    ...
    function handleSelect(e) {
      e.preventDefault();
      alert("selecting is not allowed")
    }
    ...
    const questions = async () => {
      try {
        const docRef = doc(db, "questions","cmaths","cmaths" ,`${id}`);
...
{data.begin?(
 ...
 <Box sx={{marginTop:"15%"}}>
        ...
        {data.questions.length > 0 ? ( <Typography sx={{textAlign:"center", margin:"2px"}}><b>MULTIPLE CHOICE QUESTIONS</b></Typography>):(<Typography></Typography>)}
             <ol style={{paddingLeft:"25px",marginLeft:"5px",  userSelect: 'none', WebkitUserSelect: 'none', MozUserSelect: 'none', msUserSelect: 'none' }}  onSelect={handleSelect} onCopy={handleCopy}> 
        {data.questions.map((question, index) => (
          <li key={index} style={{marginBottom:"10px",marginLeft:"5px",  userSelect: 'none', WebkitUserSelect: 'none', MozUserSelect: 'none', msUserSelect: 'none' }} onSelect={handleSelect} onCopy={handleCopy}>
            {question.question}
            <br/> 
...

        {
          data.theory.length > 0? (
            ...
            <ol style={{paddingLeft:"20px",marginLeft:"5px",  userSelect: 'none', WebkitUserSelect: 'none', MozUserSelect: 'none', msUserSelect: 'none' }}  onSelect={handleSelect} onCopy={handleCopy}>
