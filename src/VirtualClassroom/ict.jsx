...
import { Button, IconButton,Card, Toolbar, AppBar, Typography, Box } from '@mui/material';
...
function Ict() {
  ...
  const [data, setData] = useState('')
    ...
    useEffect(()=>{ 
      const  postRef=collection(db,"questions", "ict" ,"ict");
   ...
   <Box sx={{marginTop:"15%"}}>
    <Typography variant="h6">SELECT THE TEST...</Typography>
     ...
     { data && data.map(({ time,id,begin,createdAt, title})=>( 
           ...
           <Card  onClick={()=> navigate(`/ictquestions/${id}`)}  key={{id}} sx={{minHeight:"120px", width:"98%", margin:"5px", marginRight:"1%"}}>
