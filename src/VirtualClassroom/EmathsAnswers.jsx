...
function EmathsAnswers() {
  const [data, setData] = useState('')
    ...
    useEffect(()=>{ 
      const  postRef=collection(db,"answers", "emaths" ,"emaths");
   ...
   <Box sx={{marginTop:"15%"}}>
    <Typography variant="h6">SELECT THE TEST...</Typography>
     ...
     { data && data.map(({ title,id,createdAt, begin})=>( 
           ...
           <Card  onClick={()=> navigate(`/answerdetail3/${id}`)}  key={{id}} sx={{minHeight:"120px", width:"98%", margin:"5px", marginRight:"1%"}}>
