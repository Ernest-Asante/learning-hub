...
function Emaths() {
  ...
  const [data, setData] = useState('')
    ...
    useEffect(()=>{ 
      const  postRef=collection(db,"questions", "emaths" ,"emaths");
   ...
   <Box sx={{marginTop:"15%"}}>
    <Typography variant="h6">SELECT THE TEST...</Typography>
     ...
     { data && data.map(({ time,id,begin,createdAt, title})=>( 
           ...
           <Card  onClick={()=> navigate(`/emathsquestions/${id}`)}  key={{id}} sx={{minHeight:"120px", width:"98%", margin:"5px", marginRight:"1%"}}>
